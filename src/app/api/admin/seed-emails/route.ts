...
// POST /api/admin/seed-emails — Seed email sequences
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return json({
      ok: true,
      seeded: seededSequences.length,
      sequences: summary.map(row => ({
        name: row.sequence,
    ...
    console.error("Email sequences seeding error:", error);
    return json({ ok: false, error: "Failed to seed email sequences" }, 500);
  }
}

// GET /api/admin/seed-emails — List current sequences
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sequences = await sql`
      SELECT sequence, step, subject, is_active, send_count, created_at
      FROM email_sequences
