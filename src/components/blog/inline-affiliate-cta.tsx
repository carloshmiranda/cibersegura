...
interface InlineAffiliateCTAProps {
  ...
  source: string; // For UTM tracking (e.g., "blog-nis2-inline")
  articleSlug?: string; // Blog post slug for tracking
  position: number; // Position within article (1, 2, 3...)
  className?: string;
}
...
export default function InlineAffiliateCTA({
  tool,
  contextualCopy,
  source,
  articleSlug,
  ...
  // Track inline CTA impression when it becomes visible
  useCTAImpressionTracking(ctaRef, {
  ...
  const trackedUrl = addUTMParams(
    ...
    source,
    'affiliate',
    'nis2-inline-recommendations',
    `${tool.name.toLowerCase().replace(/\s+/g, '-')}-inline-${position}`
  );

  const ctaPosition = `inline-${position}`;
    ...
    <div ref={ctaRef} className={`my-8 p-5 rounded-lg border-l-3 border-accent bg-accent-light/50 ${className}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm text-brand font-medium mb-3 leading-relaxed">
            {contextualCopy}
          </p>

          <div className="flex items-start gap-3 p-4 rounded-lg border border-border bg-white hover:border-accent hover:shadow-sm transition">
            <div className="flex-shrink-0 p-1.5 rounded bg-bg-subtle text-accent">
              {getCategoryIcon(tool.category)}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                <h4 className="font-bold text-brand text-sm">
                  {tool.name}
                </h4>
                {tool.recommended && (
          ...
          <div className="mt-3 text-xs text-text-muted">
            ℹ️ Avaliação gratuita disponível — comece sem custos
          </div>
        </div>
      </div>
    </div>
